...
def create_static_model(database):
    ...
    for row in history_list:
        user, last_seen, sent, recveded = row
        user = QStandardItem(user)
        user.setEditable(False)
        ...
        recveded = QStandardItem(str(recveded))
        recveded.setEditable(False)
        list.appendRow([user, last_seen, sent, recveded])
    return list
...
class ConfigWindow(QDialog):
    ...
    def initUI(self):
        ...
        self.port_label.setFixedSize(150, 20)
        ...
        self.ip_label_note = QLabel('Пустое поля, для принятия соединения с любых адресов.', self)
